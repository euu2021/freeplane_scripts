...
| ⭐ | [MinimizedNodeTooltipOnSelection](scripts/MinimizedNodeTooltipOnSelection.groovy) | Shows a minimized (shortened) node's full content as a tooltip when the keyboard selection reaches it — the same tooltip the mouse gets on hover, without the mouse. It only pops up when a key press actually changes the selection, and it goes away on the next selection, on a click, on wheel scrolling or when the window loses focus. Running the script again switches it off. | `fold/minimize` `navigation` `listener` |
